...
def main():
    ...
    print(f"  {'seqlen':>8}  {'GFLOP':>8}  {'GB_hbm':>8}  {'AI (F/B)':>10}  {'pred ms':>9}  {'bound':>9}")
    for M in [512, 1024, 2048, 4096]:
        fl, byt, ai = flash_attn_intensity(2, 16, M, M, 64)
        ms, bound   = roofline_predict(fl, byt, gpu)
        print(f"  {M:>8}  {fl/1e9:>8.1f}  {byt/1e9:>8.2f}  {ai:>10.1f}  {ms:>9.3f}  {bound:>9}")

    # ── Fused RMSNorm + Linear ─────────────────────────────────────────
    print("\nFused RMSNorm + Linear")
    print(f"  {'config':>20}  {'AI fused':>10}  {'AI unfused':>12}  "
